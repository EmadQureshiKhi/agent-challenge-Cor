...
/**
 * Extended interface for CordUser that includes Privy functionality
...
/**
 * Loads cached CordUser data from localStorage
...
/**
 * Custom hook for managing User data fetching, caching, and synchronization
...
export function useUser(): NeurUserInterface {
  ...
  const fetcher = useCallback(async (): Promise<NeurUser | null> => {
    ...
    if (privyUser) {
      debugLog('Fetching CordUser data from server', null, {
        module: 'useUser',
        level: 'info',
      });
      const neurUser = await fetchNeurUserData(privyUser as PrivyUser);
      debugLog('Merged CordUser data', neurUser, {
        module: 'useUser',
        level: 'info',
      });
      return neurUser;
    }
    debugLog('No valid CordUser data retrieved', null, {
      module: 'useUser',
      level: 'warn',
    });
    return null;
